...
## 1.0.30：1.0.29 验收修复（`docs/changes-2026-09-12-client-guide-priceimport.md` 第 10 节）

- 所有"先写库再刷新运行态"的管理接口统一：运行态刷新失败返回 503 并列出失败子系统，不再静默返回 200；快照恢复刷新全部运行态后汇总报告。
- User-Agent 截断含省略号不超过 200 字节。

## 1.0.31：1.0.30 验收修复（`docs/changes-2026-09-12-client-guide-priceimport.md` 第 11 节）

- 路由 / 合规向量构建：向量已写库但索引重载失败返回 503，不再报成功；审计样本编辑报告构建失败。
- 合规引擎在首次成功加载规则前一律阻断（不再空规则放行）；启动时合规启用而规则读取失败则拒绝启动。
- 配置页在恢复返回 503 时仍显示价目跳过 / 合并报告。

## 1.0.32：1.0.31 验收修复（`docs/changes-2026-09-12-client-guide-priceimport.md` 第 12 节）

- 智能路由设置保存后检查网关快照重载，失败返回 503。
- 新增路由样本报告 `failed>0` 的向量构建失败。
- 配置页恢复：成功后关闭抽屉并刷新缓存（修复 1.0.31 的回归）；503 正文的恢复报告可读并展示。

## 1.0.33：1.0.32 验收修复（`docs/changes-2026-09-12-client-guide-priceimport.md` 第 13 节）

- 智能路由设置整份固化在网关快照代次里，重建失败时请求整体沿用旧设置。
- 写库成功但运行态刷新失败的 503 带 `committed` 与已创建资源；新增「重新加载运行态」接口与按钮；创建接口幂等（相同重试不再建重复账号 / 词 / 样本）。

## 1.0.34：1.0.33 验收修复（`docs/changes-2026-09-12-client-guide-priceimport.md` 第 14 节）

- 创建幂等键由数据库唯一索引强制：账号名、（策略组, 敏感词）、（策略组, 样本文本摘要）、（标签, 样本文本摘要）；升级时同名账号改名、重复词 / 样本保留最早一条。
- 账号重试比较全部生效字段，不同即 409；查询出错不再插入；命中既有样本仍执行请求的向量构建。
- 手动「重新加载运行态」失败的响应不再声称已写库，可直接重试。

## 1.0.35：1.0.34 验收修复（`docs/changes-2026-09-12-client-guide-priceimport.md` 第 15 节）

- 幂等键升级迁移：单事务 + PostgreSQL advisory lock；同名账号改名不再可能重名；重复敏感词 / 样本合并为带有效状态的一行（启用、向量、备注、阈值都不丢），被删行记日志。
- 批量导入路由样本会把既有样本一并构建向量；批量导入敏感词对既有与并发冲突幂等。
- 编辑账号名 / 敏感词撞唯一键返回 409；重复键识别基于驱动错误码。

## 1.0.36：1.0.35 验收修复（`docs/changes-2026-09-12-client-guide-priceimport.md` 第 16 节）

- 幂等键迁移在同一事务里建好唯一索引；重复样本按配置的向量身份选幸存者，不再丢掉当前可用向量。
- 批量路由样本：备注严格比较；`created` 取数据库实际插入数，被抢先的行照样构建。
- 编辑撞键后读取冲突资源失败返回 500，不伪造 409。

## 1.0.37：1.0.36 验收修复（`docs/changes-2026-09-12-client-guide-priceimport.md` 第 17 节）

- 向量身份有了唯一的解析规则（账号 Base URL + 映射后上游模型），迁移与运行态一致；空旧身份视为兼容；坏设置中止迁移。
- 幂等键索引按元数据核对确为唯一且列正确，同名错误索引在同一事务里重建。
- 批量导入路由样本：集合查询分块、一次最多 2,000 条；写入后核对失败返回带 `committed` 的 503，重提幂等。

## 1.0.38：1.0.37 验收修复（`docs/changes-2026-09-12-client-guide-priceimport.md` 第 18 节）

- 向量客户端由一条 SQL 的账号快照构造，密钥、地址、模型必属同一代次。
- 向量按产生它的快照身份存储，配置在请求飞行期间切换不会错标。
- 幂等键索引核对排除部分 / 表达式索引，不合格者在锁内重建。

## 1.0.39：1.0.38 验收修复（`docs/changes-2026-09-12-client-guide-priceimport.md` 第 19 节）

- 向量端点地址统一规范化，迁移保留运行态能装载的向量。
- 向量写回是绑定文本与代次的条件更新：构建期间被编辑 / 删除或被更新代次写过的样本不再被旧结果覆盖。
- 幂等键索引按名逐个核对，同表无关索引不再导致启动失败。

## 1.0.40：1.0.39 验收修复（`docs/changes-2026-09-12-client-guide-priceimport.md` 第 20 节）

- 向量构建改为数据库裁决的领取令牌，先开始的构建不能覆盖后开始的构建（含身份 A→B→A 的顺序）。
- 幂等键索引核对包含 collation 与排序，`NOCASE` 等同名索引会被重建为精确键。
...
## 1.0.44：1.0.43 验收修复（`internal-docs/changes-2026-09-20-custom-json-endpoints.md` 第 6 节）

- 自定义接口用量：`prompt_/completion_tokens` 与 `input_/output_tokens` 是别名，不再相加；OpenAI 命名优先；显式 0 记为已确认的零消耗，`usage` 无计数字段才是 unknown。
- 接口路径规范化按路径段去掉 `/v1` 前缀，`/v1beta/x`、`/v10/x` 保持原样；数据面派发时再核对内置前缀。
- 自定义接口的成功响应保留上游 2xx 状态码（200 / 201 / 202 …），日志同码；对话等转换协议仍统一为 200。

## 1.0.45：1.0.44 验收修复（`internal-docs/changes-2026-09-20-custom-json-endpoints.md` 第 7 节）
...
## 1.0.54：1.0.53 验收修复（`internal-docs/changes-2026-09-24-backup-restore.md` 第 6 节）

- 备份的重放边界改由数据库快照自带：计量写入器在每次提交的同一事务里记录"journal 代次:已提交偏移"，备份包的检查点取快照里的边界。保留期已清理原始明细、但仍在 journal 里的历史记录，还原后不再被重复计入小时汇总；同一份备份重复还原结果不变。
- 轮转与备份复制互斥，轮转先写新代次再截断。
- 自动备份失败时在该小时内每分钟重试；启动时清理未发布的上传目录。

## 1.0.55：1.0.54 验收修复（`internal-docs/changes-2026-09-24-backup-restore.md` 第 7 节）

- 启动时两个检查点取较新者：数据库里随提交原子写入的边界领先于文件检查点（提交后、写文件前崩溃）且落在当前文件的行边界上时以数据库为准，不再回退，保留期已清理明细的历史记录不会在重启后重复计入汇总；尚未提交的尾部仍正常重放。
