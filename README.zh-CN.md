...
| 配置版本与备份 | 账号、模型组、价目、设置每次改动前自动快照，可查看、打点、一键回滚；整机备份包（数据库快照、计量 journal、密钥）一键生成下载，每日自动备份，上传备份包或新机 `yzapi -restore` 一键还原 |
...
make smoke                     # 端到端：mock 上游 + 网关，65 项
make crud                      # 管理 / 用户 API 全量往返，93 项
```
