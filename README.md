...
<p align="center">
  ...
  <a href="docs/deploy.en.md">Deployment</a> ·
...
Model names are resolved leniently (case, vendor prefix, date suffix). `count_tokens`, CORS and `Retry-After` pass-through are supported, and every response carries `X-Upstream-Account` / `X-Upstream-Model` / `X-Upstream-Protocol`. Cross-protocol limits are documented in [docs/api.en.md](docs/api.en.md).
...
docs/               deployment guide, API contract, performance notes, release notes (*.en.md in English)
```

## Documentation

- [Deployment guide](docs/deploy.en.md): 1Panel, Docker Compose, binary + systemd, multi-instance PostgreSQL, backup and upgrade
- [API reference](docs/api.en.md): every data-plane and admin endpoint, error codes, protocol-conversion limits
- [Performance notes for coding tools](docs/coding-performance.en.md): configuration advice and how the per-call timings are defined
- [Release notes](docs/release-notes-1.0.3-1.0.4.md) (Chinese): per-version changes and upgrade notes

The Chinese originals (`docs/*.md`) are authoritative when the two differ.
